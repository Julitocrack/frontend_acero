import { useState, useEffect, useRef } from 'react' // Importamos useRef para la memoria de alertas
...
function PanelLogistica({ usuarioActual, onCerrarSesion }) {
  ...
  // Estados para visor de fotos y modal de detalles
  ...
  const pedidosYaNotificados = useRef(new Set()) // Memoria para no repetir el sonido del mismo pedido
  ...
  useEffect(() => {
    ...
    const intervalo = setInterval(() => {
      obtenerDatos() // Busca cambios en el taller automáticamente
    }, 60000)
  ...
  // ==============================================
  // MOTOR DE VIGILANCIA DE LOGÍSTICA
  // ==============================================
  useEffect(() => {
    ...
    // Filtramos los que acaban de llegar de Producción (estado En_Logistica)
    const listosParaEnviar = pedidosLogistica.filter(p => p.estado === 'En_Logistica');

    listosParaEnviar.forEach(pedido => {
      // Si el pedido está listo para logística y no hemos avisado de este ID
    ...
    if (hayNuevosParaCargar) {
      ...
      new Notification("¡Carga Lista para Ruta! 🚚", { 
        ...
        icon: "/logo-logistica.png"
      });
    }
  }, [pedidos, permisoNotificaciones]);
  ...
  const intentarFinalizarEntrega = (id) => {
    setConfirmacion({
  ...
  const pedidosLogistica = pedidos.filter(p => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 md:p-8 relative text-gray-950 overflow-x-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-slate-800 rounded-2xl shadow-xl p-5 sm:p-6 flex flex-col sm:flex-row gap-5 justify-between items-center mb-6 sm:mb-8">
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto justify-between border-t border-slate-700 pt-5 sm:border-0 sm:pt-0 shrink-0">
            ...
            {permisoNotificaciones === 'granted' && (
              <div className="bg-slate-700 p-3 rounded-full shadow-inner text-blue-400" title="Alertas de Carga Listas">
                🔔
              </div>
            )}
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 sm:p-6 md:p-8 border border-gray-100 animate-in fade-in relative">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            ...
            <button onClick={obtenerDatos} className="w-full sm:w-auto text-center text-sm bg-gray-100 text-gray-700 px-5 py-3 rounded-xl hover:bg-gray-200 transition font-semibold shadow-sm">↻ Actualizar Rutas</button>
          </div>
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    ...
                    {(pedido.estado === 'Aprobado' || pedido.estado === 'En_Produccion') && (
                      pedido.requiere_produccion ? (
                        <button disabled className="w-full bg-gray-200 text-gray-500 font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 text-base cursor-not-allowed border-2 border-gray-300 shadow-inner">
                          ⏳ Esperando cortes...
                        </button>
                      ) : (
                        <button onClick={() => intentarCargarEnvio(pedido.id)} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-extrabold py-3.5 rounded-xl transition active:scale-95 shadow-md flex items-center justify-center gap-2 text-base shrink-0 border-2 border-blue-700">
                          🚚 Cargar y Enviar
                        </button>
                      )
                    )}
                    ...
                    {pedido.estado === 'En_Logistica' && (
                      <button onClick={() => intentarFinalizarEntrega(pedido.id)} className="w-full bg-green-600 hover:bg-green-700 text-white font-extrabold py-3.5 rounded-xl transition active:scale-95 shadow-md flex items-center justify-center gap-2 text-base shrink-0 border-2 border-green-700">
                        ✅ Finalizar (Entregado)
                      </button>
                    )}
                  </div>
      ...
      {/* MODAL: Detalles, Visor de Fotos y Doble Confirmación (Omitidos para brevedad, pero iguales a los anteriores) */}
      {pedidoDetalle && (
        <div className="fixed inset-0 bg-gray-950/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-2 sm:p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="bg-slate-800 p-5 sm:p-6 flex justify-between items-center text-white shrink-0">
              ...
              <button onClick={() => setPedidoDetalle(null)} className="text-slate-400 hover:text-white text-3xl font-bold">&times;</button>
            </div>
            <div className="p-5 sm:p-8 space-y-6 overflow-y-auto bg-gray-50 flex-grow">
               <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div><p className="text-xs font-bold text-gray-400 uppercase mb-1">Origen de Venta</p><p className="font-bold text-gray-900">{getNombreSucursal(pedidoDetalle.sucursal_id)}</p></div>
                  <div className="text-right sm:text-left"><p className="text-[10px] text-gray-500 font-medium">📥 Ingresó: {formatearFechaYHora(pedidoDetalle.fecha_creacion)}</p></div>
               </div>
               <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm">
                  <div className="flex justify-between items-center mb-3">
                     <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Material y Dirección</h4>
                     <button onClick={() => setImagenAmpliando(pedidoDetalle.url_foto_ticket)} className="text-blue-600 hover:text-blue-800 text-sm font-bold flex items-center gap-1.5 bg-blue-50 px-3 py-1.5 rounded-lg transition">🔍 Ver Ticket</button>
                  </div>
      ...
      {imagenAmpliando && (
        <div className="fixed inset-0 bg-gray-950/90 backdrop-blur-md flex items-center justify-center p-4 z-[100] animate-in zoom-in duration-200">
          <button onClick={() => setImagenAmpliando(null)} className="absolute top-4 right-4 sm:top-8 sm:right-8 text-white bg-gray-800 rounded-full w-12 h-12 flex items-center justify-center text-3xl font-bold transition shadow-xl active:scale-95">&times;</button>
      ...
      {confirmacion && (
        <div className="fixed inset-0 bg-gray-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-[200] animate-in zoom-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden text-center p-8">
            <div className="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-yellow-100 mb-6"><span className="text-3xl">⚠️</span></div>
            ...
            <div className="flex gap-3">
              <button onClick={() => setConfirmacion(null)} className="flex-1 bg-gray-100 py-3.5 rounded-xl font-bold text-gray-700 hover:bg-gray-200 transition">Cancelar</button>
              <button onClick={confirmacion.accion} className={`flex-1 text-white py-3.5 rounded-xl font-bold shadow-md transition ${confirmacion.colorBoton}`}>{confirmacion.textoBoton}</button>
            </div>
          </div>
        </div>
      )}

...
function PanelDuena({ usuarioActual, onCerrarSesion }) {
  ...
  // Estados de Control (Modales y Formularios)
  const [pedidoSeleccionado, setPedidoSeleccionado] = useState(null) 
  const [pedidoDetalle, setPedidoDetalle] = useState(null) 
  
  const [decision, setDecision] = useState('Aprobado')
  const [requiereProduccion, setRequiereProduccion] = useState(false)
  ...
  const obtenerDatosIniciales = async () => {
    ...
    try {
      const [resPedidos, resSucursales, resUsuarios] = await Promise.all([
        ...
        fetch('https://aceros-backend-production.up.railway.app/usuarios/')
      ])
      ...
      if (resSucursales.ok) {
        ...
        if (listaSucs.length > 0 && !nuevoEmpleado.sucursal_id) {
          setNuevoEmpleado(prev => ({ ...prev, sucursal_id: listaSucs[0].id }))
        }
      }
    } catch (error) {
  ...
  const pedidosActivos = pedidos.filter(p => p.estado !== 'Entregado' && p.estado !== 'Rechazado')
  const pedidosHistorial = pedidos.filter(p => p.estado === 'Entregado' || p.estado === 'Rechazado')

  // ==============================================
  // MOTOR DE VIGILANCIA GERENCIAL
  // ==============================================
  useEffect(() => {
    ...
    pedidosActivos.forEach(pedido => {
      // 1. Alertar si hay pedidos nuevos esperando autorización
      ...
      // 2. Alertar si hay un pedido atorado en el taller por más de 2 horas
      if ((pedido.estado === 'Aprobado' || pedido.estado === 'En_Produccion') && pedido.fecha_aprobacion) {
        ...
        const diffHrs = Math.floor((horaActual - inicio) / (1000 * 60 * 60));
  ...
  const historialAgrupadoPorFecha = pedidosHistorial.reduce((grupos, pedido) => {
  ...
  const generarTraspaso = async (e) => {
    ...
    try {
      const respuesta = await fetch('https://aceros-backend-production.up.railway.app/pedidos/', { method: 'POST', body: formData })
      if (respuesta.ok) {
        setMostrarTraspaso(false); setArchivoTraspaso(null); setNombreArchivoTraspaso('Ningún archivo seleccionado'); setNotasTraspaso(''); setOrigenTraspaso(''); setDestinoTraspaso(''); obtenerDatosIniciales()
  ...
  const ejecutarAprobacion = async () => {
    ...
    const datosAprobacion = {
      estado: decision, requiere_produccion: decision === 'Aprobado' ? requiereProduccion : false, sucursal_destino_id: decision === 'Aprobado' ? parseInt(sucursalDestino) : null
    }
    try {
      const res = await fetch(`https://aceros-backend-production.up.railway.app/pedidos/${pedidoSeleccionado.id}/aprobar`, {
  ...
  const registrarSucursal = async (e) => {
    ...
    try {
      const res = await fetch('https://aceros-backend-production.up.railway.app/sucursales/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(nuevaSucursal) })
  ...
  const actualizarSucursal = async (e) => {
    e.preventDefault()
    try {
      const res = await fetch(`https://aceros-backend-production.up.railway.app/sucursales/${sucursalEditando.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(sucursalEditando) })
  ...
  const registrarEmpleado = async (e) => {
    ...
    try {
      const res = await fetch('https://aceros-backend-production.up.railway.app/usuarios/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({...nuevoEmpleado, sucursal_id: parseInt(nuevoEmpleado.sucursal_id)}) })
      if (res.ok) { setNuevoEmpleado({ nombre_completo: '', username: '', password: '', rol: 'ventas', sucursal_id: sucursales.length > 0 ? sucursales[0].id : '' }); obtenerDatosIniciales(); } else alert('❌ Verifica que el usuario no exista.')
    } catch (e) { alert('❌ Error') }
  ...
  const actualizarEmpleado = async (e) => {
    ...
    try {
      const res = await fetch(`https://aceros-backend-production.up.railway.app/usuarios/${empleadoEditando.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(datosAMandar) })
  ...
  const intentarEliminarEmpleado = (emp) => { setConfirmacion({ titulo: `¿Eliminar a ${emp.nombre_completo}?`, mensaje: 'Esta persona perderá acceso al sistema de inmediato. Esta acción no se puede deshacer.', textoBoton: 'Sí, Eliminar Personal', colorBoton: 'bg-red-600 hover:bg-red-700', accion: async () => { setConfirmacion(null); try { const res = await fetch(`https://aceros-backend-production.up.railway.app/usuarios/${emp.id}`, { method: 'DELETE' }); if (res.ok) obtenerDatosIniciales(); else alert('❌ Error al eliminar.'); } catch (e) { alert('❌ Error de conexión') } } }); }
  const intentarEliminarSucursal = (suc) => { setConfirmacion({ titulo: `¿Eliminar la ${suc.nombre}?`, mensaje: 'Asegúrate de que no haya empleados ni pedidos activos asignados a esta sucursal. Esta acción es irreversible.', textoBoton: 'Sí, Eliminar Sucursal', colorBoton: 'bg-red-600 hover:bg-red-700', accion: async () => { setConfirmacion(null); try { const res = await fetch(`https://aceros-backend-production.up.railway.app/sucursales/${suc.id}`, { method: 'DELETE' }); if (res.ok) obtenerDatosIniciales(); else alert('❌ Error. Es probable que aún tenga empleados o pedidos asignados.'); } catch (e) { alert('❌ Error de conexión') } } }); }
  const intentarEnviarAprobacion = (e) => { e.preventDefault(); const esAprobado = decision === 'Aprobado'; setConfirmacion({ titulo: esAprobado ? '¿Aprobar y Enviar a Taller?' : '¿Rechazar Pedido?', mensaje: esAprobado ? `¿Confirmas que la orden #${pedidoSeleccionado.id} está correcta y lista para producción?` : `¿Confirmas que deseas cancelar y rechazar la orden #${pedidoSeleccionado.id}?`, textoBoton: esAprobado ? 'Sí, Aprobar' : 'Sí, Rechazar', colorBoton: esAprobado ? 'bg-blue-600 hover:bg-blue-700' : 'bg-red-600 hover:bg-red-700', accion: () => { setConfirmacion(null); ejecutarAprobacion(); } }); }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-slate-800 rounded-2xl shadow-xl p-5 sm:p-6 flex flex-col sm:flex-row gap-5 justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto justify-between border-t border-slate-700 pt-5 sm:border-0 sm:pt-0 shrink-0">
            ...
            {permisoNotificaciones === 'granted' && (
              <div className="bg-slate-700 p-3 rounded-full shadow-inner text-yellow-400" title="Alertas de Gerencia Activadas">
                🔔
              </div>
            )}
        ...
        <div className="flex gap-2 sm:gap-4 mb-8 bg-white p-2 rounded-xl border border-gray-200 shadow-sm overflow-x-auto">
          <button onClick={() => setVistaActiva('pedidos')} className={`flex-1 sm:flex-none px-6 py-3 rounded-lg font-bold transition-all text-sm whitespace-nowrap ${vistaActiva === 'pedidos' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}>📋 Pedidos Activos</button>
          <button onClick={() => setVistaActiva('historial')} className={`flex-1 sm:flex-none px-6 py-3 rounded-lg font-bold transition-all text-sm whitespace-nowrap ${vistaActiva === 'historial' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}>🗂️ Historial</button>
          <button onClick={() => setVistaActiva('estadisticas')} className={`flex-1 sm:flex-none px-6 py-3 rounded-lg font-bold transition-all text-sm whitespace-nowrap ${vistaActiva === 'estadisticas' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}>📊 Estadísticas</button>
          <button onClick={() => setVistaActiva('personal')} className={`flex-1 sm:flex-none px-6 py-3 rounded-lg font-bold transition-all text-sm whitespace-nowrap ${vistaActiva === 'personal' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}>👥 Personal</button>
          <button onClick={() => setVistaActiva('sucursales')} className={`flex-1 sm:flex-none px-6 py-3 rounded-lg font-bold transition-all text-sm whitespace-nowrap ${vistaActiva === 'sucursales' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}>🏗️ Sucursales</button>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5 sm:p-6 md:p-8 border border-gray-100 animate-in fade-in">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
              ...
              <div className="flex gap-3 w-full sm:w-auto">
                <button onClick={obtenerDatosIniciales} className="flex-1 sm:flex-none text-center text-sm bg-gray-100 text-gray-700 px-5 py-3 rounded-xl hover:bg-gray-200 transition font-semibold"> ↻ Actualizar </button>
                <button onClick={() => setMostrarTraspaso(true)} className="flex-1 sm:flex-none text-center text-sm bg-purple-600 text-white px-5 py-3 rounded-xl font-bold shadow-md"> 📦 Nuevo Traspaso </button>
              </div>
            </div>
            {cargando && pedidosActivos.length === 0 ? (
              <div className="text-center py-20 flex flex-col items-center gap-4"> <div className="h-10 w-10 border-4 border-gray-200 border-t-gray-900 rounded-full animate-spin"></div> </div>
            ) : pedidosActivos.length === 0 ? (
              <div className="text-center py-20 bg-gray-50 rounded-xl border border-dashed border-gray-200"> <span className="text-5xl block mb-3">✅</span> <p className="text-gray-600 font-medium text-lg">No hay pedidos activos.</p> </div>
            ) : (
                ...
                {pedidosActivos.map((pedido) => (
                  <div key={pedido.id} className={`bg-white rounded-2xl overflow-hidden shadow-sm transition-all duration-300 flex flex-col animate-in slide-in-from-bottom-2 ${pedido.estado === 'Pendiente' ? 'border-yellow-400 border-2 ring-4 ring-yellow-50' : (pedido.tipo_orden === 'Traspaso' ? 'border-purple-300 border-2 shadow-inner bg-purple-50' : 'border-slate-800 border-2 shadow-inner bg-slate-50/50') }`}>
                    <div onClick={() => setImagenAmpliando(pedido.url_foto_ticket)} className="h-48 bg-gray-100 overflow-hidden relative group cursor-pointer border-b border-gray-200 shrink-0">
                      <img src={pedido.url_foto_ticket} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                      <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 flex items-center justify-center transition duration-300"> <span className="text-white text-sm bg-gray-900/80 px-4 py-2 rounded-full font-bold shadow-lg flex items-center gap-2 backdrop-blur-sm">🔍 Ver Ticket</span> </div>
                    </div>
                    <div className="p-5 flex flex-col flex-grow">
                      ...
                      <div className="mb-4">
                        ...
                        {pedido.sucursal_destino_id && pedido.estado !== 'Pendiente' && (
                           <div className="mt-3 bg-blue-50 border border-blue-100 rounded-lg p-2.5 flex items-center gap-2.5"> <span className="text-lg">🏭</span> <div> <p className="text-[9px] text-blue-500 font-black uppercase tracking-widest">Asignado a taller:</p> <p className="text-sm font-bold text-blue-900 leading-tight">{getNombreSucursal(pedido.sucursal_destino_id)}</p> </div> </div>
                        )}
                      </div>
                      <div className={`p-4 rounded-xl flex-grow mb-5 overflow-hidden ${pedido.estado === 'Pendiente' ? 'bg-yellow-100 border-2 border-yellow-200' : 'bg-gray-50 border border-gray-100'}`}> <p className="text-gray-700 text-sm leading-relaxed whitespace-pre-wrap break-words line-clamp-3"> {pedido.notas} </p> </div>
                      <div className="mt-auto pt-4 border-t border-gray-100">
                          {pedido.estado === 'Pendiente' ? (
                            <button onClick={() => { setPedidoSeleccionado(pedido); setDecision('Aprobado'); const sucursalesConTaller = sucursales.filter(s => s.tiene_produccion); setSucursalDestino(sucursalesConTaller.length > 0 ? sucursalesConTaller[0].id : ''); }} className="w-full bg-yellow-400 hover:bg-yellow-500 text-yellow-950 font-extrabold py-3.5 rounded-xl transition shadow-md text-sm flex items-center justify-center gap-2 active:scale-95 border-2 border-yellow-300"> <span>🔎</span> Revisar y Autorizar </button>
                          ) : (
                            <div className="flex flex-col gap-3">
                               {(pedido.estado === 'Aprobado' || pedido.estado === 'En_Produccion') && (
                                  <div className="flex justify-between items-center bg-slate-800 text-white px-4 py-2.5 rounded-xl shadow-inner"> <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">Tiempo en Taller:</span> <span className={`font-mono font-bold tracking-widest text-lg ${pedido.estado === 'En_Produccion' ? 'text-orange-400' : 'text-blue-300'}`}>{obtenerTiempoTranscurrido(pedido.fecha_aprobacion)}</span> </div>
                               )}
                               <button onClick={() => setPedidoDetalle(pedido)} className="w-full bg-white hover:bg-slate-50 text-slate-700 font-bold py-3 rounded-xl transition text-sm border-2 border-slate-200 shadow-sm"> 📄 Ver Detalles Completos </button>
                            </div>
                          )}
                      </div>
                    </div>
        ...
        {vistaActiva === 'historial' && (
          <div className="bg-white rounded-2xl shadow-sm p-5 sm:p-6 md:p-8 border border-gray-100 animate-in fade-in">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
              ...
              <button onClick={obtenerDatosIniciales} className="w-full sm:w-auto text-center text-sm bg-gray-100 text-gray-700 px-5 py-3 rounded-xl hover:bg-gray-200 transition font-semibold">↻ Actualizar Historial</button>
            </div>
            {Object.keys(historialAgrupadoPorFecha).length === 0 ? (
               <div className="text-center py-20 bg-gray-50 rounded-xl border border-dashed border-gray-200"><p className="text-gray-600 font-medium text-lg">El historial está vacío.</p></div>
            ) : (
                      ...
                      {historialAgrupadoPorFecha[fecha].map((pedido) => (
                        <div key={pedido.id} className="bg-slate-50/50 border-2 border-slate-800 rounded-2xl overflow-hidden shadow-inner hover:shadow-md transition-all flex flex-col">
                          <div onClick={() => setImagenAmpliando(pedido.url_foto_ticket)} className="h-32 bg-gray-200 overflow-hidden relative group cursor-pointer border-b-2 border-slate-800 shrink-0">
                            <img src={pedido.url_foto_ticket} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 flex items-center justify-center transition"> <span className="text-white text-xs bg-gray-900/80 px-3 py-1 rounded-full font-bold flex items-center gap-2">🔍 Ver Foto</span> </div>
                          </div>
                          <div className="p-4 flex flex-col flex-grow">
                            ...
                            <div className="bg-white p-3 rounded-xl border border-gray-200 flex-grow shadow-sm"> <p className="text-gray-600 text-xs leading-relaxed whitespace-pre-wrap break-words"> {pedido.notas} </p> </div>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        {/* =========================================
            VISTA 3: ESTADÍSTICAS (SÚPER PRO)
            ========================================= */}
        {vistaActiva === 'estadisticas' && (() => {
          ...
          const ventasPorSucursal = entregadosDelMes.reduce((acc, p) => {
             ...
             acc[sucId] = (acc[sucId] || 0) + 1; return acc;
          }, {});
          ...
          const cortesPorTaller = entregadosDelMes.filter(p => p.requiere_produccion && p.sucursal_destino_id).reduce((acc, p) => {
            const tallerId = parseInt(p.sucursal_destino_id);
            acc[tallerId] = (acc[tallerId] || 0) + 1; return acc;
          }, {});
          ...
          const tiemposSucursal = entregadosDelMes.filter(p => p.fecha_aprobacion && p.estado === 'Entregado').reduce((acc, p) => {
            ...
            if(diffMs > 0) {
              acc[sucId].sum += diffMs; acc[sucId].count += 1;
            }
            return acc;
          }, {});
          ...
          const tendenciaDiaria = entregadosDelMes.reduce((acc, p) => {
            const dia = new Date(p.fecha_creacion).getDate();
            acc[dia] = (acc[dia] || 0) + 1; return acc;
          }, {});
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                 <div className="bg-slate-50/50 border-2 border-slate-800 rounded-3xl p-6 sm:p-8 flex flex-col justify-between shadow-inner">
                    <div>
                       ...
                       <h3 className="text-slate-900 text-center font-black text-5xl tracking-tight">{totalEntregados}</h3>
                    </div>
                 ...
                 <div className="bg-slate-50/50 border-2 border-slate-800 rounded-3xl p-6 sm:p-8 flex flex-col justify-between shadow-inner">
                    <div>
                       ...
                       <h3 className="text-purple-950 text-center font-black text-5xl tracking-tight">{totalTraspasosMes} <span className="text-2xl font-black text-gray-500">Traspasos</span></h3>
                    </div>
                 ...
                 <div className="bg-slate-50/50 border-2 border-slate-800 rounded-3xl p-6 sm:p-8 flex flex-col justify-between shadow-inner">
                    <div>
                       ...
                       <h3 className="text-orange-950 text-center font-black text-5xl tracking-tight">{tasaCierreVentas}%</h3>
                    </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="bg-white border-2 border-slate-800 rounded-3xl p-6 shadow-inner">
                      ...
                      <div className="space-y-4">
                        {rankingVentas.map(([sucId, total], index) => {
                            const sucObj = sucursales.find(s => s.id === parseInt(sucId));
                            const porcentaje = (total / maxVentasSucursal) * 100;
                            return (
                  ...
                  <div className="bg-white border-2 border-slate-800 rounded-3xl p-6 shadow-inner">
                      ...
                      <div className="space-y-4">
                        {rankingTrabajoTaller.map(([tallerId, total], index) => {
                            const sucObj = sucursales.find(s => s.id === parseInt(tallerId));
                            const porcentaje = (total / maxCortesTaller) * 100;
                            return (
...
{'(Verde: Eficiente / Rojo: Retraso)'}</p>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        {eficienciaGrid.map(({ sucId, avgTimeHrs }) => {
                          const sucObj = sucursales.find(s => s.id === sucId);
                          return (
                            <div key={sucId} className={`p-4 rounded-xl border-2 text-center flex flex-col justify-center items-center shadow-inner ${
                               avgTimeHrs <= 4 ? 'bg-emerald-100 border-emerald-300' : 
                               avgTimeHrs <= 12 ? 'bg-yellow-100 border-yellow-300' : 'bg-red-100 border-red-300'
                            }`}>
                  ...
                  <div className="bg-white border-2 border-slate-800 rounded-3xl p-6 shadow-inner">
                      ...
                      <div className="flex flex-col gap-2.5 max-h-[300px] overflow-y-auto pr-1 text-sm text-gray-600">
                        {Object.entries(tendenciaDiaria).sort(([a], [b]) => b - a).map(([dia, total]) => (
                           <p key={dia} className="p-2.5 border border-gray-100 bg-gray-50 rounded-lg flex justify-between font-medium"><span>Día {dia} del mes:</span> <b>{total} material entregado(s)</b></p>
                        ))}
                      </div>
                  </div>
              </div>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in">
                 ...
                 {empleados.map(emp => (
                   <div key={emp.id} className="p-4 border border-gray-200 rounded-xl bg-gray-50 hover:bg-white hover:shadow-md transition-all flex justify-between items-center gap-2">
                     ...
                     <div className="flex flex-col gap-2 shrink-0">
                       <button onClick={() => setEmpleadoEditando(emp)} className="p-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition" title="Editar">✏️</button>
                       <button onClick={() => intentarEliminarEmpleado(emp)} className="p-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition" title="Eliminar">🗑️</button>
                     </div>
                   </div>
                 ))}
               </div>
             ...
             <div className="bg-white rounded-2xl shadow-sm p-5 sm:p-8 border border-gray-100 h-fit">
               ...
               <form onSubmit={registrarEmpleado} className="space-y-5">
                 <input type="text" value={nuevoEmpleado.nombre_completo} onChange={(e) => setNuevoEmpleado({...nuevoEmpleado, nombre_completo: e.target.value})} className="w-full border p-3 rounded-xl bg-gray-50 outline-none focus:ring-2 focus:ring-blue-100" placeholder="Nombre Completo" required />
                 <div className="grid grid-cols-2 gap-4">
                   <input type="text" value={nuevoEmpleado.username} onChange={(e) => setNuevoEmpleado({...nuevoEmpleado, username: e.target.value})} className="w-full border p-3 rounded-xl bg-gray-50 outline-none" placeholder="Usuario" required />
                   <input type="password" value={nuevoEmpleado.password} onChange={(e) => setNuevoEmpleado({...nuevoEmpleado, password: e.target.value})} className="w-full border p-3 rounded-xl bg-gray-50 outline-none" placeholder="Contraseña" required />
                 </div>
                 <div className="grid grid-cols-2 gap-4">
                   ...
                   <select value={nuevoEmpleado.rol} onChange={(e) => setNuevoEmpleado({...nuevoEmpleado, rol: e.target.value})} className="w-full border p-3 rounded-xl bg-white outline-none cursor-pointer">
                     <option value="ventas">📝 Ventas</option><option value="produccion">⚙️ Producción</option><option value="logistica">🚚 Logística</option><option value="duena">⚖️ Gerencia</option>
                   </select>
                 </div>
                 <button type="submit" disabled={creandoUsuario} className="w-full bg-blue-600 text-white font-bold py-3.5 rounded-xl shadow">➕ Registrar</button>
               </form>
             </div>
           </div>
         )}
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in">
               ...
               <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
                 {sucursales.map(suc => (
                   <div key={suc.id} className="p-4 border border-gray-200 rounded-xl bg-gray-50 hover:bg-white hover:shadow-md transition-all flex justify-between items-center gap-4">
                     <div>
                         <div className="flex flex-wrap items-center gap-2.5 mb-0.5">
                            <p className="font-bold text-gray-950 text-lg">🏗️ {suc.nombre}</p>
                                ...
                                <span className="inline-flex items-center gap-1 bg-slate-800 text-white text-[10px] uppercase tracking-wider font-bold px-2.5 py-1 rounded-md shadow-sm whitespace-nowrap">
                                ⚙️ Con Producción
                            </span>
                            )}
                        </div>
                     ...
                     <div className="flex flex-col gap-2 shrink-0">
                       <button onClick={() => setSucursalEditando(suc)} className="p-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition" title="Editar">✏️</button>
                       <button onClick={() => intentarEliminarSucursal(suc)} className="p-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition" title="Eliminar">🗑️</button>
                     </div>
                   </div>
                 ))}
               </div>
             ...
             <div className="bg-white rounded-2xl shadow-sm p-5 sm:p-8 border border-gray-100 h-fit">
               ...
               <form onSubmit={registrarSucursal} className="space-y-5">
                 <input type="text" value={nuevaSucursal.nombre} onChange={(e) => setNuevaSucursal({...nuevaSucursal, nombre: e.target.value})} className="w-full border p-3.5 rounded-xl bg-gray-50 outline-none focus:ring-2 focus:ring-blue-100" placeholder="Nombre Comercial" required />
                 <div className="grid grid-cols-2 gap-4">
                   <input type="text" value={nuevaSucursal.direccion} onChange={(e) => setNuevaSucursal({...nuevaSucursal, direccion: e.target.value})} className="w-full border p-3.5 rounded-xl bg-gray-50 outline-none" placeholder="Dirección (Opcional)" />
                   <input type="text" value={nuevaSucursal.telefono} onChange={(e) => setNuevaSucursal({...nuevaSucursal, telefono: e.target.value})} className="w-full border p-3.5 rounded-xl bg-gray-50 outline-none" placeholder="Teléfono (Opcional)" />
                 </div>
                 ...
                 <button type="submit" disabled={creandoSucursal} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl shadow mt-2 transition">🏗️ Crear Sucursal</button>
               </form>
             </div>
           </div>
         )}
        ...
        <div className="fixed inset-0 bg-gray-950/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-2 sm:p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden max-h-[90vh] flex flex-col">
             <div className="bg-purple-800 p-5 flex justify-between items-center text-white shrink-0">
              ...
              <button onClick={() => setMostrarTraspaso(false)} className="text-purple-300 hover:text-white text-3xl font-bold">&times;</button>
            </div>
            <form onSubmit={generarTraspaso} className="p-5 sm:p-8 space-y-6 overflow-y-auto">
              <div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                <button type="button" onClick={() => setMostrarTraspaso(false)} className="w-full sm:w-auto px-6 py-3.5 bg-gray-100 hover:bg-gray-200 rounded-xl font-semibold text-gray-700">Cancelar</button>
                <button type="submit" disabled={enviandoTraspaso} className="w-full sm:w-auto px-8 py-3.5 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-bold shadow transition">{enviandoTraspaso ? 'Generando...' : '🚚 Enviar a Logística'}</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="bg-yellow-500 p-5 sm:p-6 flex justify-between items-center text-yellow-950 border-b border-yellow-600 shrink-0">
              <h3 className="font-extrabold text-xl sm:text-2xl">
                🔎 Autorizar Orden #{pedidoSeleccionado.id}
              </h3>
              <button onClick={() => setPedidoSeleccionado(null)} className="text-yellow-800 hover:text-yellow-950 text-3xl font-bold">&times;</button>
            </div>
            
            <form onSubmit={intentarEnviarAprobacion} className="p-5 sm:p-8 space-y-6 overflow-y-auto">
              ...
              <div>
                ...
                <select value={decision} onChange={(e) => setDecision(e.target.value)} className="w-full border border-gray-200 rounded-xl p-4 bg-white text-xl font-bold outline-none focus:ring-4 focus:ring-yellow-300">
                  <option value="Aprobado">✅ Aprobar y Mandar a Cortar</option><option value="Rechazado">❌ Rechazar / Cancelar</option>
                </select>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-gray-100 mt-8">
                <button type="button" onClick={() => setPedidoSeleccionado(null)} className="w-full sm:w-auto px-6 py-3.5 bg-gray-100 hover:bg-gray-200 rounded-xl font-semibold">Cancelar</button>
                <button type="submit" disabled={procesando} className="w-full sm:w-auto px-8 py-3.5 bg-blue-600 text-white rounded-xl font-bold shadow hover:bg-blue-700 transition">💾 Guardar Decisión</button>
              </div>
            </form>
          </div>
        </div>
         ...
         <div className="fixed inset-0 bg-gray-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
           <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
             <div className="bg-blue-800 p-5 flex justify-between items-center text-white">
               ...
               <button onClick={() => setEmpleadoEditando(null)} className="text-xl">&times;</button>
             </div>
             <form onSubmit={actualizarEmpleado} className="p-6 space-y-4">
               <input type="text" value={empleadoEditando.nombre_completo} onChange={e => setEmpleadoEditando({...empleadoEditando, nombre_completo: e.target.value})} className="w-full border p-3 rounded-xl" placeholder="Nombre" required />
               <input type="text" value={empleadoEditando.username} onChange={e => setEmpleadoEditando({...empleadoEditando, username: e.target.value})} className="w-full border p-3 rounded-xl" placeholder="Usuario" required />
               <input type="password" onChange={e => setEmpleadoEditando({...empleadoEditando, password: e.target.value})} className="w-full border p-3 rounded-xl" placeholder="Nueva Contraseña (Opcional)" />
               <div className="grid grid-cols-2 gap-3">
                 ...
                 <select value={empleadoEditando.rol} onChange={e => setEmpleadoEditando({...empleadoEditando, rol: e.target.value})} className="w-full border p-3 rounded-xl bg-white">
                   <option value="ventas">Ventas</option><option value="produccion">Producción</option><option value="logistica">Logística</option><option value="duena">Gerencia</option>
                 </select>
               </div>
               <div className="flex gap-3 pt-4"><button type="button" onClick={() => setEmpleadoEditando(null)} className="flex-1 bg-gray-100 py-3 rounded-xl font-bold text-gray-700 hover:bg-gray-200">Cancelar</button><button type="submit" className="flex-1 bg-blue-600 py-3 rounded-xl font-bold text-white shadow hover:bg-blue-700">💾 Guardar</button></div>
             </form>
           </div>
         </div>
       )}
         ...
         <div className="fixed inset-0 bg-gray-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
           <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
             <div className="bg-blue-800 p-5 flex justify-between items-center text-white">
               ...
               <button onClick={() => setSucursalEditando(null)} className="text-xl">&times;</button>
             </div>
             <form onSubmit={actualizarSucursal} className="p-6 space-y-4">
               <input type="text" value={sucursalEditando.nombre} onChange={e => setSucursalEditando({...sucursalEditando, nombre: e.target.value})} className="w-full border p-3 rounded-xl outline-none" placeholder="Nombre Comercial" required />
               <div className="grid grid-cols-2 gap-3">
                 <input type="text" value={sucursalEditando.direccion || ''} onChange={e => setSucursalEditando({...sucursalEditando, direccion: e.target.value})} className="w-full border p-3 rounded-xl outline-none" placeholder="Dirección" />
                 <input type="text" value={sucursalEditando.telefono || ''} onChange={e => setSucursalEditando({...sucursalEditando, telefono: e.target.value})} className="w-full border p-3 rounded-xl outline-none" placeholder="Teléfono" />
               </div>
               ...
               <div className="flex gap-3 pt-4"><button type="button" onClick={() => setSucursalEditando(null)} className="flex-1 bg-gray-100 py-3 rounded-xl font-bold text-gray-700 hover:bg-gray-200">Cancelar</button><button type="submit" className="flex-1 bg-blue-600 py-3 rounded-xl font-bold text-white shadow hover:bg-blue-700">💾 Guardar</button></div>
             </form>
           </div>
         </div>
       )}

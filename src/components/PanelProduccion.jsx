...
function PanelProduccion({ usuarioActual, onCerrarSesion }) {
  ...
  // ✅ Reloj y refresco de datos separados — dos responsabilidades, dos efectos
  useEffect(() => {
  ...
  useEffect(() => {
    obtenerDatos()
    const intervaloDatos = setInterval(obtenerDatos, 60000)
  ...
  const intentarCompletarCorte = useCallback((id) => {
    setConfirmacion({
  ...
  // ✅ useMemo — el filtro no se recalcula en cada render
  ...
  // ✅ useCallback — función estable que no se recrea cada render
  const calcularTiempoTranscurrido = useCallback((fechaIso) => {
    ...
    const texto = diffHrs > 0 ? `${diffHrs}h ${diffMins}m en cola` : `${diffMins} minutos en cola`
    return { texto, horas: diffHrs, minutos: diffMins, critico: esCritico }
  }, [horaActual])

  // ✅ Motor de vigilancia — ya NO depende de horaActual, solo de pedidosProduccion y permiso
  useEffect(() => {
    ...
    pedidosProduccion.forEach(pedido => {
      const fechaRef = pedido.fecha_aprobacion || pedido.fecha_creacion
      if (!fechaRef) return
      const inicio = new Date(fechaRef.endsWith('Z') ? fechaRef : fechaRef + 'Z')
      const diffMs = new Date() - inicio
      const diffHrs = Math.floor(diffMs / (1000 * 60 * 60))
      const diffMins = Math.floor((diffMs % (1000 * 60 * 60)) / (1000 * 60))

      if (diffHrs === 0 && diffMins <= 5 && !pedidosYaNotificados.current.has(`${pedido.id}-nuevo`)) {
        hayNuevos = true
        pedidosYaNotificados.current.add(`${pedido.id}-nuevo`)
      }
      if (diffHrs >= 2 && !pedidosYaNotificados.current.has(`${pedido.id}-critico`)) {
        hayCriticos = true
        pedidosYaNotificados.current.add(`${pedido.id}-critico`)
      }
    })
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 md:p-8 relative text-gray-950">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-slate-800 rounded-2xl shadow-xl p-5 sm:p-6 flex flex-col sm:flex-row gap-5 justify-between items-center mb-6 sm:mb-8">
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto justify-between border-t border-slate-700 pt-5 sm:border-0 sm:pt-0 shrink-0">
            ...
            {permisoNotificaciones === 'granted' && (
              <div className="bg-slate-700 p-3 rounded-full shadow-inner text-emerald-400 min-h-[44px] min-w-[44px] flex items-center justify-center" title="Alertas Activadas">
                🔔
              </div>
            )}
          ...
          {cargando && pedidosProduccion.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {pedidosProduccion.map((pedido) => {
                // ✅ Se calcula una sola vez por tarjeta — no se llama dos veces
                  ...
                  <div key={pedido.id} className={`bg-slate-50/50 border-2 rounded-2xl overflow-hidden shadow-inner hover:shadow-md transition-all duration-300 flex flex-col animate-in slide-in-from-bottom-2 ${tiempo.critico ? 'border-red-500 ring-4 ring-red-50 bg-red-50/50' : 'border-slate-800'}`}>
                    ...
                    <div className="p-5 flex-grow flex flex-col">
                      ...
                      <button
                        ...
                        className={`w-full text-white font-extrabold py-3.5 rounded-xl transition duration-200 active:scale-95 shadow-md flex items-center justify-center gap-2 text-base shrink-0 min-h-[44px] ${tiempo.critico ? 'bg-red-600 hover:bg-red-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}
                      >
      ...
      {/* ✅ Modal detalles — sin IIFE, tiempo calculado limpiamente */}
      {pedidoDetalle && (() => {
          ...
          <div className="fixed inset-0 bg-gray-950/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-2 sm:p-4 z-50 animate-in fade-in">
            <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col">
              <div className="bg-slate-800 p-5 sm:p-6 flex justify-between items-center text-white shrink-0">
                <div>
                  <h3 className="font-extrabold text-xl sm:text-2xl flex items-center gap-2">📄 Detalles de Corte #{pedidoDetalle.id}</h3>
              ...
              <div className="p-5 sm:p-8 space-y-6 overflow-y-auto overscroll-contain bg-gray-50 flex-grow">
                <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  ...
                  <div className={`px-5 py-3 rounded-xl text-center shadow-inner min-w-[140px] ${tiempo.critico ? 'bg-red-100 text-red-900' : 'bg-gray-900 text-white'}`}>
                    ...
                    <p className={`font-mono text-xl font-bold ${tiempo.critico ? 'animate-pulse' : 'text-orange-400'}`}>{tiempo.texto}</p>
                  </div>
                </div>
                ...
                <button
                  ...
                  className={`w-full text-white font-extrabold py-4 rounded-xl transition duration-200 active:scale-95 shadow-md flex items-center justify-center gap-2 text-lg mt-4 min-h-[44px] ${tiempo.critico ? 'bg-red-600 hover:bg-red-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}
                >

...
function PanelVentas({ usuarioActual, onCerrarSesion }) {
  ...
  // NUEVO: Estado para el Sistema de Doble Confirmación
  ...
  const obtenerDatosIniciales = async () => {
    ...
    try {
      ...
      let sucs = []
      if (resSucursales.ok) {
          sucs = await resSucursales.json()
          setSucursales(sucs)
      }

      if (resPedidos.ok) {
        const todosLosPedidos = await resPedidos.json()
        const misPedidos = todosLosPedidos.filter(p => p.sucursal_id === usuarioActual.sucursal_id)
        setSolicitudes(misPedidos)
      }
  ...
  const miSucursal = sucursales.find(s => s.id === parseInt(usuarioActual.sucursal_id));
  const sucursalTieneTaller = miSucursal ? miSucursal.tiene_produccion : false;

  const solicitudesActivas = solicitudes.filter(p => p.estado !== 'Entregado' && p.estado !== 'Rechazado')
  const solicitudesHistorial = solicitudes.filter(p => p.estado === 'Entregado' || p.estado === 'Rechazado')

  const historialAgrupadoPorFecha = solicitudesHistorial.reduce((grupos, pedido) => {
  ...
  const handleFileChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        const res = await fetch('https://aceros-backend-production.up.railway.app/pedidos/analizar-ticket', {
          method: 'POST',
          body: formDataInfo
        });
        if (res.ok) {
      ...
      } catch (err) { console.error("No se pudo leer el ticket con IA"); }
      setLeyendoIA(false);
    }
  }

  // PASO 1: INTERCEPTAMOS EL CLIC DE ENVIAR PARA MOSTRAR LA CONFIRMACIÓN
  const intentarEnviarPedido = (e) => {
    ...
    setConfirmacion({
      ...
      accion: ejecutarEnvioPedido // Si dice que sí, ejecuta esta función
    });
  }

  // PASO 2: EJECUTA EL ENVÍO REAL SI EL USUARIO CONFIRMÓ
  const ejecutarEnvioPedido = async () => {
    setConfirmacion(null); // Cerramos el modal
    setEnviando(true);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 md:p-8 relative text-gray-950">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-slate-800 rounded-2xl shadow-xl p-5 sm:p-6 flex flex-col sm:flex-row gap-5 justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-5 w-full sm:w-auto justify-between border-t border-slate-700 pt-5 sm:border-0 sm:pt-0 shrink-0">
            ...
            <button onClick={intentarCerrarSesion} className="bg-red-500 hover:bg-red-600 text-white font-bold py-3 px-5 rounded-xl transition active:scale-95 text-sm shadow-md shrink-0">
              Cerrar Sesión
            </button>
          </div>
        </div>
        ...
        {/* Pestañas */}
        <div className="flex gap-2 sm:gap-4 mb-8 bg-white p-2 rounded-xl border border-gray-200 shadow-sm">
          <button onClick={() => setVistaActiva('activas')} className={`flex-1 sm:flex-none px-6 py-3 rounded-lg font-bold transition-all text-sm whitespace-nowrap ${vistaActiva === 'activas' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}>📋 Solicitudes en Curso</button>
          <button onClick={() => setVistaActiva('historial')} className={`flex-1 sm:flex-none px-6 py-3 rounded-lg font-bold transition-all text-sm whitespace-nowrap ${vistaActiva === 'historial' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}>🗂️ Historial de Ventas</button>
        </div>

        {/* CONTENIDO DE LA VISTA */}
        <div className="bg-white rounded-2xl shadow-sm p-5 sm:p-6 md:p-8 border border-gray-100 animate-in fade-in">
          <button onClick={() => setMostrarFormulario(true)} className="sm:hidden fixed bottom-6 right-6 h-16 w-16 bg-orange-600 text-white rounded-full shadow-2xl flex items-center justify-center text-3xl z-40 active:scale-95 transition-transform">+</button>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              <button onClick={obtenerDatosIniciales} className="flex-1 sm:flex-none text-center text-sm bg-gray-100 text-gray-700 px-5 py-3 rounded-xl hover:bg-gray-200 transition font-semibold">
                ↻ Actualizar
              </button>
              <button onClick={() => setMostrarFormulario(true)} className="flex-1 sm:flex-none text-center text-sm bg-orange-600 hover:bg-orange-700 text-white px-5 py-3 rounded-xl font-bold transition shadow active:scale-95">
                + Pedir Material
              </button>
            </div>
          </div>
          ...
          {cargando ? (
            <div className="text-center py-20 flex flex-col items-center gap-4"><div className="h-10 w-10 border-4 border-gray-200 border-t-orange-600 rounded-full animate-spin"></div><p className="text-gray-500 font-medium">Cargando...</p></div>
          ) : (vistaActiva === 'activas' ? (
                  ...
                  {solicitudesActivas.map(solicitud => (
                    <div key={solicitud.id} className="bg-white border rounded-2xl overflow-hidden shadow-sm flex flex-col">
                       <div onClick={() => setImagenAmpliando(solicitud.url_foto_ticket)} className="h-40 bg-gray-100 overflow-hidden relative group cursor-pointer border-b shrink-0">
                          <img src={solicitud.url_foto_ticket} className="w-full h-full object-cover" />
        ...
        <div className="fixed inset-0 bg-gray-950/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-2 sm:p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden animate-in slide-in-from-bottom sm:zoom-in duration-300 flex flex-col max-h-[95vh]">
            <div className="bg-orange-600 p-5 flex justify-between items-center text-white shrink-0">
              ...
              <button onClick={() => setMostrarFormulario(false)} className="text-orange-200 hover:text-white text-3xl font-bold h-10 w-10 flex items-center justify-center rounded-full hover:bg-orange-700 transition">&times;</button>
            </div>
            
            {/* FORMULARIO CON LA FUNCIÓN INTERCEPTADA */}
            <form onSubmit={intentarEnviarPedido} className="p-5 sm:p-8 space-y-6 bg-white overflow-y-auto flex-grow relative text-left">
              {leyendoIA && (
              ...
              <div className="space-y-6">
                  <div>
                    ...
                    <input type="text" value={numeroTicket} onChange={(e) => setNumeroTicket(e.target.value)} className="w-full border border-gray-300 rounded-xl p-4 text-gray-950 bg-yellow-50 focus:ring-2 focus:ring-orange-300 outline-none font-black text-xl shadow-inner placeholder:text-gray-300" placeholder="Ej. 12939" required />
                  ...
                  <div>
                    ...
                    <textarea value={notasNuevoPedido} onChange={(e) => setNotasNuevoPedido(e.target.value)} className="w-full border border-orange-200 rounded-2xl p-4 text-gray-950 text-base bg-orange-50/50 placeholder:text-gray-400 focus:ring-2 focus:ring-orange-300 transition resize-none h-48 outline-none shadow-inner whitespace-pre-wrap" placeholder="Sube la foto o escribe aquí el material a cortar..." required></textarea>
                  </div>
              </div>

              <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4 mt-4 bg-white border-t border-gray-100">
                <button type="button" onClick={() => setMostrarFormulario(false)} className="w-full sm:w-auto px-6 py-3.5 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-xl transition font-semibold text-base active:scale-95 flex items-center justify-center gap-2">Cancelar</button>
                <button type="submit" disabled={enviando} className="w-full sm:w-auto px-10 py-3.5 bg-orange-600 hover:bg-orange-700 disabled:bg-gray-400 text-white rounded-xl transition font-bold text-base shadow active:scale-95 flex items-center justify-center gap-2.5">
      ...
      {/* ==============================================
          NUEVO: MODAL GLOBAL DE DOBLE CONFIRMACIÓN
          ============================================== */}
      {confirmacion && (
